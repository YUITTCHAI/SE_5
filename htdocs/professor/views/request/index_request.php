		...
		<table class="shadow table table-hover">
  			<thead class="thead-dark">
    			<tr align='center' >
				 ...
				 <th scope="col">จัดการคำขอฝึกงาน</th>
   			   </tr>
  			</thead></tr>
...
<?php foreach($request_list as $request)
{
	...
	if($request->status == "waiting"){
		echo"<td data-lable='จัดการคำขอฝึกงาน'><a href=?controller=request&action=updateForm&idrequest=$request->idrequest class='btn btn-info btn' role='button' ata-placement='right' title='จัดการคำขอฝึกงาน'><i class='fas fa-folder-open'></i> จัดการคำขอฝึกงาน</a></td>";
	}
	if($request->status == "approve" || $request->status == "decline"){
		echo"<td data-lable='รายละเอียด'><a href=?controller=request&action=detailRequest&idrequest=$request->idrequest class='btn btn-warning btn' role='button' ata-placement='right' title='รายละเอียด'><i class='fas fa-folder-open'></i> รายละเอียด</a></td>";
	}
}
echo "</table>";
?>
